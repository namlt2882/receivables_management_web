// User
export const FETCH_USERS = 'FETCH_USERS';
export const ADD_USER = 'ADD_USER';
export const UPDATE_USER = 'UPDATE_USER';
export const DELETE_USER = 'DELETE_USER';
export const EDIT_USER = 'EDIT_USER';
export const FILTER_TABLE = 'FILTER_TABLE';
export const SORT_TABLE = 'SORT_TABLE';

// MessageForm
export const FETCH_MESSAGES = 'FETCH_MESSAGES';
export const ADD_MESSAGE = 'ADD_MESSAGE';
export const SEARCH_MESSAGE = 'SEARCH_MESSAGE';

// Profile
export const FETCH_PROFILES = 'FETCH_PROFILES';
export const GET_PROFILE = 'GET_PROFILE';
export const ADD_PROFILE = 'ADD_PROFILE';
export const EDIT_PROFILE = 'EDIT_PROFILE';
export const DELETE_PROFILE = 'DELETE_PROFILE';

//Process
export const NEW_PROCESS = 'NEW_PROCESS';
export const SET_PROCESS = 'SET_PROCESS';
export const EDIT_PROCESS = 'EDIT_PROCESS';
export const DELETE_PROCESS = 'DELETE_PROCESS';
export const SET_PROCESS_EDITABLE = 'SET_PROCESS_EDITABLE';
export const SET_PROCESS_UNEDITABLE = 'SET_PROCESS_UNEDITABLE';
export const SAVE_CACHE = 'SAVE_CACHE';
export const LOAD_CACHE = 'LOAD_CACHE';

//Stage
export const ADD_STAGE = 'ADD_STAGE';
export const EDIT_STAGE = 'EDIT_STAGE';
export const DELETE_STAGE = 'DELETE_STAGE';
export const INCREMENT_STAGE_ORDER = 'INCREMENT_STAGE_ORDER';
export const DECREMENT_STAGE_ORDER = 'DECREMENT_STAGE_ORDER';

//Action
export const ADD_ACTION = 'ADD_ACTION';
export const EDIT_ACTION = 'EDIT_ACTION';
export const DELETE_ACTION = 'DELETE_ACTION';

//Receivable
export const FETCH_RECEIVABLE_LIST = 'FETCH_RECEIVABLE_LIST';
export const ADD_RECEIVABLE = 'ADD_RECEIVABLE';
export const EDIT_RECEIVABLE = 'EDIT_RECEIVABLE';
export const GET_RECEIVABLE = 'GET_RECEIVABLE';
export const DELETE_RECEIVABLE = 'DELETE_RECEIVABLE';

//Receivable status
export const SET_RECEIVABLE_EDITABLE = 'SET_RECEIVABLE_EDITABLE'
export const SET_RECEIVABLE_UNEDITABLE = 'SET_RECEIVABLE_UNEDITABLE'